...
import '../../models/titled_model.dart';
...
class AuthService implements AuthServiceInterface {
  ...
  @override
  Future handleSignup(tilted) async {
    print('%%%%%%%% handleSignup %%%%%%%%');
    ...
    if (tilted != null) {
      tilted.id = user.uid;
      tilted.createdAt = Timestamp.now();
      tilted.username =
          user.phoneNumber != "null" ? user.phoneNumber : user.email;
      tilted.notificationEnabled = true;
      tilted.phone = user.phoneNumber;
      tilted.email = user.email;
      tilted.status = 'active';
      tilted.connected = true;
      tilted.country = 'Brasil';

      DocumentReference cusRef =
          FirebaseFirestore.instance.collection('customers').doc(user.uid);

      await cusRef.set(Titled().toJson(tilted: tilted));
    }
  ...
  Future<String> verifyNumber(String userPhone) async {
    ...
    await _auth.signInWithPhoneNumber(phoneMobile);
